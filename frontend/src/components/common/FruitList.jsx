...
export default function FruitList({
  ...
  const [ratingSellerId, setRatingSellerId] = useState(null);
  ...
  const handleRemove = async (fruitId) => {
    try {
      ...
      const txRequest = await contract.removeFruit.populateTransaction(fruitId);
      ...
      setStatus("Fruit removed successfully.");

      if (refreshFruits) {
        await refreshFruits();
      }
    } catch (error) {
      handleWeb3Error(error, setStatus, "remove fruit");
    } finally {
  ...
  return (
    <div style={{ marginTop: "20px" }}>
      {fruits.length === 0 ? (
        ...
        >
          {fruits.map((fruit) => {
            ...
            const isHighlighted =
              highlightId && Number(highlightId) === fruit.id;
                      ...
                      !fruit.alreadyRated && (
                        ...
                        >
                          <RateSellerForm
                            ...
                            account={account}
                            refreshFruits={refreshFruits}
                            ratingSellerId={ratingSellerId}
                            setRatingSellerId={setRatingSellerId}
                          />
                        </div>
                      )}
                    ...
                    {mode === "seller" && isSeller && !isDeleted && (
                      ...
                      >
                        <button
                          ...
                          disabled={
                            updatingFruitId !== null || deletingFruitId !== null
                          }
                        >
                        ...
                        <button
                          ...
                          disabled={
                            deletingFruitId !== null || updatingFruitId !== null
                          }
                        >
                          {deletingFruitId === fruit.id
                            ? "Deleting..."
                            : "Delete"}
                        </button>
                      </div>
      ...
      <DeleteConfirmModal
        ...
        isDeleting={deletingFruitId !== null}
        onClose={() => setFruitToDelete(null)}
        onConfirm={async () => {
          if (!fruitToDelete || deletingFruitId !== null) return;
      ...
      <UpdateFruitModal
        ...
        account={account}
        refreshFruits={refreshFruits}
        updatingFruitId={updatingFruitId}
